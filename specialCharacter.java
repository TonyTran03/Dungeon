...
public class specialCharacter extends nameCreation {
    ...
    public void statrolls() {
        ...
        for(int i = 0; i<22; i++){ //points generator 
            int dice = ran.nextInt(4); //.nextInt is inclusive of 0 but exclusive of value- pick a random number in the range
            String iv = roller[dice]; // made an instance of the array, selecting a random index [0,4)
            stats.put(iv, stats.get(iv)+1 ); // we use .put method again to reassign a new key to the value. This works because "iv" variable shares the same name as the key. In this case,we overwrite the previous value (key, value) to be +1
        }
